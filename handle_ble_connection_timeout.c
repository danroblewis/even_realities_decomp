...

{
  ...
  uVar3 = (uint)CONNECTION_STATE_VALIDATION_BUFFER;
  ...
  if (iVar1 != 0) {
    schedule_ble_connection_timeout_with_priority
              (iVar2,&LAB_00073728_1,BLE_CONNECTION_STATE_VALIDATION_BUFFER + -1,
               BLE_CONNECTION_STATE_VALIDATION_BUFFER + -1 >> 0x1f);
    return;
  }
  return;
}
