/*
 * Function: FUN_0005d964
 * Entry:    0005d964
 * Prototype: undefined FUN_0005d964()
 */


int FUN_0005d964(int param_1)

{
  undefined1 uVar1;
  int iVar2;
  int iVar3;
  undefined4 uVar4;
  undefined1 *puVar5;
  int iVar6;
  uint uVar7;
  int iVar8;
  
  if (*(char *)(param_1 + 3) != '\x01') {
    return -0x16;
  }
  iVar2 = FUN_0008307a();
  if (iVar2 == 0) {
    return -0x80;
  }
  iVar8 = iVar2 + 4;
  iVar3 = FUN_00082ff6(iVar8,4);
  if (iVar3 != 0) {
    return -5;
  }
  iVar3 = FUN_00082ff6(iVar8,3);
  if ((iVar3 != 0) || (iVar3 = FUN_00082ff6(iVar8,1), iVar3 != 0)) {
    return -0x10;
  }
  iVar3 = FUN_0005cac0(iVar2);
  uVar7 = (uint)*(byte *)(*(int *)(iVar2 + 0xf0) + 10);
  if (uVar7 == 3) {
    iVar6 = FUN_0005caec(iVar2);
    if (iVar6 != 3) goto LAB_0005da60;
    if (iVar3 != 0) {
      uVar7 = *(uint *)(iVar3 + 0xc);
      goto LAB_0005da5c;
    }
  }
  else if (uVar7 < 4) {
    if (uVar7 - 1 < 2) goto LAB_0005da60;
  }
  else if ((uVar7 == 4) &&
          ((iVar6 = FUN_0005caec(iVar2), iVar6 != 3 ||
           ((iVar3 != 0 && (*(int *)(iVar3 + 0xc) != 0)))))) {
    uVar7 = (uint)DAT_2001d532;
LAB_0005da5c:
    if (uVar7 != 0) goto LAB_0005da60;
  }
  iVar3 = FUN_00082ff6(param_1 + 4,0xb);
  if (iVar3 == 0) {
    if (*(int *)(param_1 + 0xc0) == 0) {
      iVar3 = FUN_0005e7c8(0x20,*(undefined1 *)(param_1 + 8),param_1 + 0x90);
      *(int *)(param_1 + 0xc0) = iVar3;
      if (iVar3 == 0) {
        uVar4 = FUN_0005e7c8(4,*(undefined1 *)(param_1 + 8),param_1 + 0x90);
        *(undefined4 *)(param_1 + 0xc0) = uVar4;
      }
    }
    iVar3 = *(int *)(param_1 + 0xc0);
    if (((iVar3 != 0) && ((*(ushort *)(iVar3 + 0xe) & 0x24) != 0)) &&
       ((*(byte *)(param_1 + 10) < 3 ||
        (((int)((uint)*(byte *)(iVar3 + 0xd) << 0x1f) < 0 &&
         ((*(byte *)(param_1 + 10) == 3 ||
          (((int)((uint)*(ushort *)(iVar3 + 0xe) << 0x1a) < 0 && (*(char *)(iVar3 + 0xc) == '\x10'))
          )))))))) {
LAB_0005da60:
      if (*(int *)(param_1 + 0xc0) == 0) {
        iVar3 = FUN_0005e6a8(*(undefined1 *)(param_1 + 8),param_1 + 0x90);
        *(int *)(param_1 + 0xc0) = iVar3;
        if (iVar3 == 0) {
          return -0xc;
        }
      }
      iVar3 = FUN_0005cc30(iVar2);
      if ((iVar3 == 0) && (iVar3 = FUN_000830b0(iVar2,0xb), iVar3 != 0)) {
        puVar5 = (undefined1 *)FUN_0005f5d0(iVar3 + 0xc,1);
        uVar1 = FUN_0005d568(iVar2,9);
        *puVar5 = uVar1;
        iVar6 = FUN_00081820(param_1,6,iVar3,0,0);
        if (iVar6 == 0) {
          FUN_00083090(iVar8,0xf);
          FUN_00083090(iVar2,1);
          return 0;
        }
        FUN_0005f24c(iVar3);
        return iVar6;
      }
      return -0x69;
    }
  }
  return -0x16;
}


