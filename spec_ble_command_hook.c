...

{
  ...
  iVar3 = compare_byte_arrays(&local_520,&CMD_TEST_BLE_UNPAIR);
  if (iVar3 == 0) {
    if (2 < LOG_LEVEL) {
      if (IS_DEBUG == 0) {
  ...
  if (iVar3 != 0) {
    ...
    safe_string_copy_with_overflow_check(&value,pcVar9,(int)pcVar13 - (int)pcVar9,0x20);
    safe_string_copy(&jstr_len,pcVar13 + 1,0x80);
    if (2 < LOG_LEVEL) {
      if (IS_DEBUG == 0) {
...
LAB_0000f2fc:
              ...
              enabled = calculate_string_length(&jstr_len);
              if (2 < LOG_LEVEL) {
                if (IS_DEBUG == 0) {
...
LAB_0000f3f4:
          ...
          if (sec == 0x4b) {
            ...
            safe_string_copy_with_overflow_check(&local_5c0,&value,(int)pcVar9 - (int)&value,0x20);
            safe_string_copy(&jstr_len,pcVar9 + 1,0x80);
            if (2 < LOG_LEVEL) {
              if (IS_DEBUG == 0) {
...
LAB_0000f7ec:
                  handle_buffer_overflow_condition(&jstr_len,0,200,pos,uVar14,iVar3,iVar5,uVar12);
LAB_0000f9d4:
                  enabled = 200;
...
LAB_0000f87e:
                ...
                if (jstr_len == 0) {
                  ...
                  handle_buffer_overflow_condition(&jstr_len,0,0x14,"base: %d",iVar3);
                  (**(code **)(sec + 0xc))(&jstr_len,0x14);
                  goto LAB_0000f70c;
                }
...
LAB_0000f932:
                  ...
                  if (jstr_len == 0) {
                    ...
                    handle_buffer_overflow_condition
                              (&jstr_len,0,200,
                               ...
                               ,&BUILD_DATE_STRING,"2024-12-28 20:20:45",1,4,5,
                               VERSION_INFO_POINTER_ALT);
                    goto LAB_0000f9d4;
                  }
                  ...
                  if (jstr_len == 0) {
                    ...
                    enabled = get_adc_nfc_current_value();
                    handle_buffer_overflow_condition(&jstr_len,0,0x40,"VADC:%d TMR:%d",enabled,0);
LAB_0000fa4a:
                    enabled = 0x40;
                    pcVar11 = *(code **)(sec + 0xc);
                    goto LAB_0000f9d8;
