/*
 * Function: process_ble_connection_data_with_framebuffer
 * Entry:    0005eeb4
 * Prototype: undefined process_ble_connection_data_with_framebuffer()
 */


int process_ble_connection_data_with_framebuffer
...
LAB_0005eeda:
    ...
    iVar6 = check_connection_state_validity(iVar9);
    if (iVar6 == 0) {
      DEBUG_PRINT2("ASSERTION FAIL [%s] @ %s:%d\n","z_spin_lock_valid(l)",
      ...
      update_connection_state_flags(iVar9);
      if (*(ushort *)(param_1 + 0x22) == 0) {
        uVar11 = validate_and_clear_connection_state(iVar9);
        if ((int)uVar11 != 0) {
          bVar2 = (bool)isCurrentModePrivileged();
          if (bVar2) {
...
LAB_0005f02e:
          ...
          setup_framebuffer(&local_30,(int)((ulonglong)uVar11 >> 0x20),iVar7,uVar4);
          iVar7 = (**(code **)**(undefined4 **)(&DAT_20003aa8 + (uint)*(byte *)(iVar9 + 10) * 0x34))
                            (iVar9,&local_24,local_30,uStack_2c);
          *(int *)(iVar9 + 0x14) = iVar7;
          if (iVar7 == 0) {
            setup_logging_system(&DAT_20003a7c + (uint)*(byte *)(iVar9 + 10) * 0x34,iVar9);
            return 0;
          }
