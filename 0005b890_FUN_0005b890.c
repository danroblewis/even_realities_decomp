/*
 * Function: FUN_0005b890
 * Entry:    0005b890
 * Prototype: undefined FUN_0005b890()
 */


undefined4 FUN_0005b890(int param_1,undefined4 param_2,int param_3)

{
  int iVar1;
  int iVar2;
  undefined2 *puVar3;
  undefined *puVar4;
  undefined *puVar5;
  uint uVar6;
  undefined4 local_28;
  char *local_24;
  
  if (*(int *)(param_1 + 8) != 0x5a465) {
    return 1;
  }
  puVar5 = *(undefined **)(param_1 + 0xc);
  if (puVar5 == &DAT_20002a20) {
    iVar1 = 0;
    puVar4 = &DAT_2000ff19;
    do {
      iVar2 = FUN_000826b2(puVar4,&DAT_000f2b3a);
      if (iVar2 == 0) {
        iVar2 = FUN_00056f08(puVar4[-1],puVar4,7);
        if (iVar2 == 0) {
          puVar3 = *(undefined2 **)(*(int *)(param_3 + 0x10) + 0x10);
          FUN_0005a128(puVar4[-1],puVar4,*puVar3,puVar3[1]);
        }
        else {
          FUN_000566a4();
        }
      }
      iVar1 = iVar1 + 1;
      puVar4 = puVar4 + 0xc;
    } while (iVar1 != 3);
  }
  if (*(short *)(puVar5 + 8) != *(short *)(param_3 + 0xc)) {
    return 1;
  }
  iVar1 = FUN_00056e74(*puVar5,puVar5 + 1);
  if (iVar1 == 0) {
    return 1;
  }
  if ((*(char *)(iVar1 + 0xd) == '\a') &&
     ((*(code **)(puVar5 + 0x14) == (code *)0x0 ||
      (iVar2 = (**(code **)(puVar5 + 0x14))(iVar1,param_1), iVar2 != 0)))) {
    iVar2 = FUN_00082cba(iVar1,param_1,0x94);
    if (iVar2 == 0) {
      if (*(short *)(param_3 + 0xc) == 2) {
        uVar6 = *(ushort *)(puVar5 + 8) & 2;
        if ((*(ushort *)(puVar5 + 8) & 2) != 0) {
          iVar2 = FUN_0005b270(iVar1,*(undefined2 *)(param_3 + 4),*(undefined4 *)(param_3 + 0x10));
          if (iVar2 != 0) goto LAB_0005b9a4;
          *(char *)(*(int *)(param_3 + 0x10) + 0x16) =
               *(char *)(*(int *)(param_3 + 0x10) + 0x16) + '\x01';
          uVar6 = 0;
        }
      }
      else {
        if (*(short *)(param_3 + 0xc) != 1) {
          FUN_000566a4(iVar1);
          *(undefined4 *)(param_3 + 8) = 0;
          return 1;
        }
        uVar6 = *(ushort *)(puVar5 + 8) & 1;
        if ((*(ushort *)(puVar5 + 8) & 1) != 0) {
          iVar2 = FUN_0005b680(iVar1,*(undefined2 *)(param_3 + 4),*(undefined4 *)(param_3 + 0x10));
LAB_0005b9a4:
          FUN_000566a4(iVar1);
          *(int *)(param_3 + 8) = iVar2;
          if (-1 < iVar2) {
            return 1;
          }
          return 0;
        }
      }
      FUN_000566a4(iVar1);
      *(uint *)(param_3 + 8) = uVar6;
      return 1;
    }
    local_24 = "Link is not encrypted";
    local_28 = 2;
    FUN_00082a42(&DAT_00088128,0x1080,&local_28);
  }
  FUN_000566a4(iVar1);
  return 1;
}


