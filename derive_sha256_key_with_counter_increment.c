/*
 * Function: derive_sha256_key_with_counter_increment
 * Entry:    0007be24
 * Prototype: undefined __stdcall derive_sha256_key_with_counter_increment(undefined4 password, undefined4 param_2, undefined4 param_3, undefined4 param_4, undefined4 param_5, undefined4 param_6, undefined4 param_7, undefined4 param_8)
 */


int derive_sha256_key_with_counter_increment
              (undefined4 password,undefined4 param_2,undefined4 param_3,undefined4 param_4,
              undefined4 param_5,undefined4 param_6,int param_7,uint param_8)

{
  int iVar1;
  char cVar2;
  uint uVar3;
  int iVar4;
  undefined4 local_13c;
  undefined1 auStack_138 [8];
  undefined1 auStack_130 [8];
  undefined4 local_128;
  undefined4 uStack_124;
  undefined4 local_120;
  undefined4 local_11c;
  undefined1 auStack_118 [48];
  undefined4 local_e8;
  undefined4 local_e4;
  int local_e0;
  undefined1 local_dc;
  undefined4 local_d8;
  undefined4 local_c0;
  undefined4 uStack_bc;
  undefined4 local_b8;
  undefined4 local_a4;
  undefined1 auStack_a0 [128];
  
  fill_memory_buffer(auStack_118,0,0x74);
  local_a4 = 0;
  fill_memory_buffer(auStack_a0,0,0x80);
  local_128 = 0;
  uStack_124 = 0;
  local_11c = 0;
  local_120 = 0;
  local_13c = 0x84;
  if (0xff0 < param_8) {
    return 0xf1000f;
  }
  copy_sha256_state_with_memory_operations();
  local_e0 = prepare_hmac_sha256_input_data
                       (param_3,param_4,param_5,param_6,&local_a4,&local_13c,param_8);
  if (local_e0 != 0) {
    return 0xf1000e;
  }
  local_e4 = 7;
  local_dc = 2;
  cVar2 = '\x01';
  local_c0 = 0;
  uStack_bc = 1;
  local_b8 = 1;
  local_e8 = param_2;
  local_d8 = password;
  fill_memory_buffer_with_zeros_wrapper(auStack_118,0x10);
  iVar1 = process_sha256_block_with_parameter_management
                    (&local_a4,local_13c,auStack_138,&local_128,0x10,auStack_130);
  iVar4 = iVar1;
  if (iVar1 == 0) {
    do {
      local_a4 = CONCAT31(local_a4._1_3_,cVar2);
      iVar1 = process_sha256_context_with_special_case_handling
                        (auStack_118,auStack_138,auStack_130,local_13c);
      if (iVar1 != 0) goto LAB_0007bf0c;
      uVar3 = param_8;
      if (0xf < param_8) {
        uVar3 = 0x10;
      }
      copy_memory_safe(param_7 + iVar4,auStack_118,uVar3);
      fill_memory_buffer_with_zeros_wrapper(auStack_118,0x10);
      cVar2 = cVar2 + '\x01';
      param_8 = param_8 - uVar3;
      iVar4 = iVar4 + uVar3;
    } while (param_8 != 0);
    iVar1 = 0;
  }
LAB_0007bf0c:
  fill_memory_buffer_utility(auStack_118,0,0x74);
  copy_sha256_state_with_memory_operations();
  return iVar1;
}


