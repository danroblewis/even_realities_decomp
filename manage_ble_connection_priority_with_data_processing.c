/*
 * Function: manage_ble_connection_priority_with_data_processing
 * Entry:    0007cb50
 * Prototype: undefined manage_ble_connection_priority_with_data_processing()
 */


void manage_ble_connection_priority_with_data_processing(void)

{
  ...
  if (uVar8 == 0) {
    ...
    iVar4 = check_connection_state_validity(&DAT_2000b490);
    if (iVar4 == 0) {
      ...
      *(byte *)((int)DMIC_BUFFER_SIZE + 0xd) = *(byte *)((int)DMIC_BUFFER_SIZE + 0xd) & 0x7f;
      remove_ble_connection_from_list(&DMIC_BUFFER_POINTER);
