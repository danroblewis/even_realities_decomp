...

{
  ...
  iVar2 = process_data_with_callback_and_parameter_alt(0x10);
  if (iVar2 != 0) {
  ...
  *(undefined4 *)(pcVar3 + 0x1014) = uVar4;
  initialize_work_mode_buffer_and_flag();
...
LAB_00017372:
    ...
    initialize_data_structure(pcVar3 + 0x77c);
    power_for_panel();
    if (*pcVar3 == '\x01') {
    ...
    check_system_status_and_copy_data(*(undefined4 *)(WORK_MODE + 0xfec));
    getDashboardStartUpModeInfofromFlash();
    getAppLanguageInfofromFlash();
    process_component_init_messages();
  ...
  iVar2 = check_driver_ready(&PTR_s_regulators_00087cb0);
  if (iVar2 == 0) {
    pcVar8 = "pmic regulator not ready.\n";
  }
  else {
    DEBUG_PRINT("pmic regulator %s is ready to use.\n","regulators");
    iVar2 = check_driver_ready(&PTR_s_charger_00087d70);
    if (iVar2 == 0) {
      pcVar8 = "Charger device not ready.\n";
    }
    else {
      iVar2 = process_imu_sensor_data_and_temperature(&PTR_s_charger_00087d70);
      if (iVar2 < 0) {
        ...
        if (-1 < iVar2) {
          ...
          initialize_system_timers_and_whitelist();
          if (1 < LOG_LEVEL) {
            if (IS_DEBUG == 0) {
              DEBUG_PRINT("%s(): exit\n","component_init");
            }
