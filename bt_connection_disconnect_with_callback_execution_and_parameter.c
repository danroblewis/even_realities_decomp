/*
 * Function: bt_connection_disconnect_with_callback_execution_and_parameter
 * Entry:    000570a0
 * Prototype: undefined bt_connection_disconnect_with_callback_execution_and_parameter()
 */
...
void bt_connection_disconnect_with_callback_execution_and_parameter(int param_1)

{
  ...
  if (*(char *)(param_1 + -0x53) == '\0') {
    ...
    while( true ) {
      if (&DAT_00088058 < puVar1) {
        ...
        assertion_failure("WEST_TOPDIR/zephyr/subsys/bluetooth/host/conn.c",0x5fe);
      }
    ...
    validate_and_process_ble_characteristics_with_callback_validation_and_state_management(iVar4);
    return;
  }
  ...
  if (iVar2 << 0x16 < 0) {
    ...
    iStack_28 = bt_connection_disconnect_with_validation_and_state_management(iVar4,&local_20);
    if (iStack_28 == 0) {
