...

{
  ...
  if (*(char *)(param_4 + 1) == '\x04') {
    safe_string_copy_with_padding(*(int *)(param_1 + 0xff8) + 8,param_2 + 0x54,0x10);
    safe_string_copy_with_padding(*(int *)(param_1 + 0xff8) + 0x19,param_2 + 0x94,0x96);
    ...
    *puVar2 = 0;
    ...
    if ((*(uint *)(param_2 + 0x1b4) < 2) &&
       (iVar1 = validate_app_whitelist(param_2 + 0x14), 1 < (iVar1 - 2U & 0xff))) {
      if (iVar1 == 1) {
        process_data_with_validation_and_retry(param_2 + 4);
      }
    }
    else {
