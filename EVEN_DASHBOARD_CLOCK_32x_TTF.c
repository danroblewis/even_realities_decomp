...

{
  ...
  handle_buffer_overflow_condition
            (local_38,0,0x10,"%d%d%s%d%d",uVar4,sVar5,":",(local_3c & 0xffff) / 10,
             (local_3c & 0xffff) % 10);
  iVar1 = calculate_string_length(local_38);
  switch(param_6) {
  case 0:
    uVar6 = 0;
    break;
  case 1:
    ...
    if (iVar2 << 0x1e < 0) {
      _clean_fb_data(MAIN_FRAMEBUFFER,0,param_2,param_3,param_4,param_5);
    }
    pbVar7 = local_38;
    iVar2 = param_2;
